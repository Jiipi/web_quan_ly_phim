...
interface HeroBannerProps {
  ...
  onAdd?: (item: HeroBannerItem) => void;
}
...
export function HeroBanner({ items, className, onPlay, onAdd }: HeroBannerProps) {
  ...
  return (
    ...
    >
      {/* Backdrop image with smooth transition */}
      <div className="absolute inset-0 overflow-hidden">
        <AnimatePresence mode="wait">
          {backdropUrl ? (
            ...
            <motion.div
              key="backdrop-empty"
              className="h-full w-full bg-gradient-to-br from-primary/20 via-bg to-secondary/10"
            />
          )}
        </AnimatePresence>
      </div>
      ...
      <div className="relative z-10 p-6 sm:p-8 md:p-10">
        <AnimatePresence mode="wait">
          ...
          >
            {/* Poster */}
            <div className="hidden h-48 w-32 shrink-0 overflow-hidden rounded-xl border border-primary/50 shadow-[0_0_24px_oklch(0.72_0.32_330_/_0.45)] md:block lg:h-56 lg:w-36">
              {posterUrl ? (
                <Image
                  src={posterUrl}
                  alt={item.title}
            ...
            <div className="flex flex-1 flex-col gap-3 md:gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2 pt-2">
                ...
                {onPlay && item.status === "watching" && (
                  <Button size="lg" variant="secondary" onClick={() => onPlay(item)}>
                    <Play className="h-4 w-4" />
                    Tiếp tục
                  </Button>
                )}
                {onAdd && (
                  <Button size="lg" variant="glass" onClick={() => onAdd(item)}>
      ...
      {/* Manual slide controllers & indicator dots */}
      {items.length > 1 && (
        <>
          <button
            type="button"
            onClick={() => setCurrentIndex((prev) => (prev - 1 + items.length) % items.length)}
            className="absolute left-4 top-1/2 z-20 -translate-y-1/2 rounded-full border border-white/10 bg-black/45 p-2 text-white/70 transition-all hover:bg-black/70 hover:text-white opacity-0 group-hover:opacity-100 shadow-[0_0_12px_rgba(0,0,0,0.5)]"
          ...
          <button
            ...
            className="absolute right-4 top-1/2 z-20 -translate-y-1/2 rounded-full border border-white/10 bg-black/45 p-2 text-white/70 transition-all hover:bg-black/70 hover:text-white opacity-0 group-hover:opacity-100 shadow-[0_0_12px_rgba(0,0,0,0.5)]"
            aria-label="Phim sau"
          >
