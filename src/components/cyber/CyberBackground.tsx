...
export function CyberBackground({ className, subtle = false }: CyberBackgroundProps) {
  ...
  return (
    <>
      {/* Light Mode subtle premium background */}
      <div
        aria-hidden="true"
        className={cn(
          "pointer-events-none fixed inset-0 -z-10 block overflow-hidden dark:hidden bg-bg",
          className,
        )}
      >
        ...
        {/* Soft side accent glows (teal & gold) */}
        <div
          className="absolute top-1/4 -left-40 h-[400px] w-[400px] rounded-full blur-3xl opacity-[0.04]"
          style={{
            background: "radial-gradient(circle, var(--accent), transparent 70%)",
          }}
        />
        <div
          className="absolute bottom-1/4 -right-40 h-[450px] w-[450px] rounded-full blur-3xl opacity-[0.04]"
          style={{
      ...
      {/* Dark Mode cyberpunk background */}
      <div
        aria-hidden="true"
        className={cn(
          // Cyber backdrop chỉ chạy khi dark mode
